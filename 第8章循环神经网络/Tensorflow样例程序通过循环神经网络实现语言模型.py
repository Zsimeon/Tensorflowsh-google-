...
from tensroflow.models.rnn.ptb import reader 
...
KEEP_PROB = 0.5						#截断不被dropout的概率
MAX_GRAD_NORM = 5					#用于控制梯度膨胀的参数

#通过一个PTBModel类来描述模型，这样方便维护循环神经网络中的状态
class PTBModel(object):
	def __init__(self, is_training, btach_size, num_steps):
		#记录使用的batch大小和截断长度
		...
		#定义预期输出，维度和ptb_iterator函数输出的正确答案的维度也是一样的
		...
		embedding = tf.get_variable("embadding", [VOCAB_SIZE,HIDDEN_SIZE])

		#将原本batch_size*num_steps个单词ID转化为单词向量，转化后的输入层维度为
		...
		#定义输出列表。在这里先将不同时刻LSTM结构的输出收集起来，在通过一个全连接层得到最终的输出
		outputs = []
		...
		with tf.variable_scope("RNN"):
			for time_step in range(num_steps):
				...
				#从输入数据中获取当前时刻获得的输入并传入LSTM结构
				cel_output,state = cell(inputs[:,time_step,:],state)
				#将当前输出加入输出队列
		...
		bias = tf.get_variable("bias",[VOCAB_SIE])
		...
		loss = tf.nn.seq2seq.sequence_loss_by_example(
			...
			#这里将[batch_size*num_steps]二维数组压缩成一维数组
			#损失的权重。在这里所有的权重为1，也就是说不同batch和不同时刻的重要程度是一样的
		...
		#通过clip_by_norm函数控制梯度大小，避免梯度膨胀的问题
...
def run_epoch(session,model,data,train_op,output_log):
	...
	for step,(x,y) in enumerate(
		...
		#在当前batch上运行train_op并计算损失值。交叉熵函数计算的就是下一个单词为给定单词的概率
		cost,state,_ = session.run(
			[model.cost, model.final_state,train_op],
			{model.input_data:x,model.targets:y,
			model.initial_state:state})
		#将不同时刻、不同batch的概率加起来就可以得到第二个perplexity公式等号右边的部分，
		#再将找哥哥和做指数运算就可以得到perplexity值
		total_costs += cost 
...
def main(_):
	...
	with tf.variable_scope("language_model",
		...
		train_model = PTBModel(False,EVAL_BATCH_SIZE,EVAL_NUM_STEP)
