...
from tensorfloe.examples.tutorials.mnist import input_data
...
def inference(input_tensor, avg_class, weights1, biases1, weights2, biases2):
    ...
    if avg_class == None:
        ...
        #使用的是不同类别对应节点输出值得相对大小，有没有softmax层对最后分类结果的计算
        #没有影响，于是在计算整个神经网络的前向传播时可以不加入最后的softmax层
...
def train(mnist):
    ...
    #GraphKeys.TRAINABLE_VARIABLES中的元素，这个集合的元素就是所有没有指定trainables=False的参数
    variable_averages_op = variable_averages.apply(tf.trainable_variables())

    #计算使用了滑动平均之后的前向传播结果，滑动平均不会改变变量本身的取值，而是会维护一个影子变量来记录其滑动平均值
    #所以当需要使用这个滑动平均值时，需要调用average函数
    average_y = inference(
        x, variable_averages,weights1,biases1,weights2,biases2    )

    #计算交叉熵作为刻画预测值和真实值之间差距的损失函数，这里使用了Tensorflow中提供的
    #sparse_soft_max_cross_entropy_with_logits函数来计算交叉熵。当分类问题只有一个正确答案时，
    #可以使用这个函数来加速交叉熵的计算。MNIST问题的图片中只包含了0~9中的一个数字，
    # 所以可以使用这个函数来计算交叉熵损失。这个函数的第一个参数是神经网络不包括softmax层的前向传播结果，
    # 第二个是训练数据的正确答案，因为标准答案时一个长度为10的一维数组，而该函数需要提供的是一个
    # 正确答案的数字，所以需要使用tf.argmax函数来得到正确答案对应的类型编号
...
with tf.Session() as sess:
    ...
    for i in range(TRAINING_STEPS):
        ...
        if i % 1000 ==0:
            ...
            print("Afetr %d training step(s),validation accuracy"
...
test_acc = sess.run(accuracy,feed_dict=test_feed)
#输出正确率信息
print("After %d training steps(s),validation accuracy using average"
