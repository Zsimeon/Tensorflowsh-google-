...
# 本小节将介绍如何利用ImageNet数据集上训练好的Inception-v3模型来解决一个新的图像
# 分类问题。可以保留训练好的模型中所有卷积层的参数，只是替换最后一层全连接层
# 在这个最后一层全连接层之前的网络层称之为瓶颈层

# 本小节将给出一个完整的TF程序来介绍如何通过TF实现迁移学习
#以下代码给出了如何下载这一小节中将要用到的数据集

curl http://download.tensorflow.org/example_images/flower_photos.tgz
tar xzf flower_photos.tgz
#解压之后为5个文件夹，每一个子文件夹的名称都是一种华的名字，代表不同类别

#通过下面的命令可以下载谷歌提供的训练好的Inception-v3模型
...
#当新的数据集和已经训练好的模型都准备好之后，可以通过以下代码来完成迁移学习
#-*- coding: utf-8 -*-
...
#Inception-v3模型中代表瓶颈层结果的张量名称，在谷歌提供的Inception-v3模型中，这个
#张量名称就是'pool_3/reshape:0'。在训练的模型时，可以通过tensor.name来获取张量的名称
...
#特征向量保存在文件中，免去重复的计算。下面的变量定义了这些文件的存放地址
...
#TEST_PRECENTAGE和VALIDATION_PERCENTAGE参数指定了测试数据集和验证数据集的大小
def create_image_lists(testing_percentage, validation_precentage):
	#得到的所有图片都存在result这个字典里，这个字典的key为类别的名称
	#value也是一个字典，字典里存储了所有图片名称
	result = {}
	#获取当前目录下所有的子目录
	...
	for sub_dir in sub_dirs:
		...
		for file_name in file_list:
			...
			elif chance < (teating_percentage + validation_percentage):
...
#这个函数通过类别名称、所属数据集和图片编号获取经过Inception-v3模型处理之后的
#特征向量文件地址
...
def run_bottleneck_on_image(sess,image_daa,image_data_tensor,bottleneck_tensor):
	#这个过程实际上就是将当前图片作为输入急撒U呢瓶颈的张量
	#这个瓶颈张量的值就是这张图片的新的特征向量
	bottleneck_values = sess.run(bottleneck_tensor,
		{image_data_tensor:image_data})
	#经过卷积神经网络处理的结果是一个四维数组，
	#需要将这个结果压缩成一个特征向量（一维数组）
	bottleneck_values = np.squeeze(bottleneck_values)
	return bottleneck_values

#这个函数获取一张图片经过Inception-v3模型处理之后的特征向量，
#这个函数会先试图寻找已经计算且保存下来的特征向量，如果找不到则
#先计算这个特征向量，然后保存到文件
def get_or_create_bottleneck(
		sess,image_lists,label_name,index,
		category,jpeg-data_tensor,bottleneck_tensor):
	#获取一张图片对应的特征向量文件的路径
	label_lists = image_lists[label_name]
	sub_dir = label_lists['dir']
	...
	#如果这个文件不存在，则通过Inception-v3模型来计算特征向量
	#并将计算结果存入文件
...
def get_random_cached_bottlenecks(
	...
	for _ in range(how_many):
		...
		bottelneck = get_or_create_bottleneck(
			sess,image_lists,label_name,image_index,category,
			jpeg_data_tensor,bottelneck_tensor)
...
#	读取已经训练好的Inception-v3模型
	...
	#定义新的神经网络输入，这个输入是新图片经过Inception-v3模型前向传播到达瓶颈层时
	#的节点取值，可以将这个过程类似地理解为一种特征提取
	...
	#定义一层全连接层来解决新的图片分类问题，因为训练好的Inception-v3模型以及把原始的图片抽象为了
	#更加容易分类的特征向量了，所以不需要再训练复杂的神经网络来完成这个新的分类任务
		...
		for i in range(STEPS):
			...
			if i % 100 == 0 or i + 1 == STEPS:
				validation_bottlenecks,validation_ground_truth = \
					get_random_cached_bottlenecks(
						sess,n_classses,image_lists,BATCH,
