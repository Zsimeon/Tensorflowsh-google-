...
'''
定义数据存储的路径，因为需要为不同的GPU提供不同的训练数据，所以通过placeholder的方式就需要手动准备多份数据
为了方便训练数据的获取过程，采用输入队列的方式，从TFRecord中读取数据。于是这里提供的数据文件路径为将MNIST
训练数据转化为TFRecord格式之后的路径
'''
...
'''
 ...
 loss的集合，如果不通过命令空间就会将不同GPU上的正则化损失都加进来
'''
...
def main(argv = None):
	...
	with tf.Graph().as_default(),tf.device('/cpu:0'):
		...
		global_step = tf.get_variable(
			'global_step', [], intializer=tf.constant_initializer(0),
			trainable=False)
		...
		for i in range(N_GPU):
			...
			with tf.device('/gpu:%d' % i):
				with tf.name_scope('GPU_%d' % i) as scope:
					...
					#在第一次声明变量后，将控制变量重用的参数设置为True。
					#这样可以让不同的GPU更新同一组参数。注意tf.name_scope函数
					#并不会影响tf.get_variable的命名空间
		...
		variable_averages = tf.train.ExponentialMovingAverage(
			MOVING_AVERAGE_DEACY,global_step)
		...
		with tf.Session(config=tf.ConfigProto(
			...
			for step in range(TRAINING_STEPS):
				...
				if step != 0 and step %10 == 0:
					...
					都会使用一个batch的训练数据，所有总共用到的训练数据个数为
					batch大小*GPU个数
					'''
					...
					'''
					example_pre_sec = num_examples_per_step / duration

					'''
					duration为运行当前训练过程使用的时间，因为在每一个训练过程中，
					每一个GPU都会使用一个batch的训练数据，所有在单个batch上的训练
					所需的时间为duration/GPU个数
					'''
					...
					if step % 1000 == 0 or (step + 1) == TRAINING_STEPS:
						...
						saver.save(sess, checkoint_path,global_step = step)
