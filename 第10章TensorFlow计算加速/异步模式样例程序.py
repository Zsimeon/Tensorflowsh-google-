...
'''
通过flags指定运行的参数，在10.4.1小节对不同的任务给出了不同的程序
但这不是一种可扩展的方式。在这一小节中将使用运行程序时给出的参数来
配置在不同任务中运行的程序
'''
...
tf.app.flags.DEFINE_string(
 	'ps_hosts','tf-ps0:2222,tf-ps1:111',
...
'''
定义TensorFlow的计算图，并返回每一轮迭代时需要运行的操作，这个过程与5.5节
中的主程序基本一致，但为了使处理分布式计算的部分更突出，本小节将此过程整理为一个函数
'''
def build_model(x,y_,is_chief):
	regularizer = tf.contrib.layers.l2_regularizer(REGULARAZTION_RATE)
	#通过和5.5节给出的mnist_inference.py代码计算神经网络前向传播结果
...
def main(argv = None):
	...
	with tf.device(tf.train.replica_device_setter(
		...
		#执行迭代过程，在迭代过程中tf.trian.Supervisor会帮助输出日志并保存模型
		#所以不需要直接调用这些过程
		while not sv.should_stop():
			...
			if step > 0 and step % 100 == 0:
				...
				#不同的计算服务器都会更新全局训练轮数，所以这里用
				#global_step_value可以直接得到在训练中使用过的batch总数
